    ...
    public partial class Bus : Form
    {
        ...
        private void bus_Load(object sender, EventArgs e)
        {
            ...
            {//打开菜单配置数据库连接
                ...
                conn.Open();//打开连接
                myadp = new MySqlDataAdapter("select * from bus", conn);
                myds = new DataSet();//填充和绑定数据
                myadp.Fill(myds, "bus");
                dataGridView.AutoGenerateColumns = false;//不生成新的列
                dataGridView.DataSource = myds.Tables["bus"];
                //把数据赋给名称对应的列并显示

                dataGridView.Columns["bus_index"].DataPropertyName = myds.Tables["bus"].Columns["bus_index"].ToString();
                dataGridView.Columns["sys_num"].DataPropertyName = myds.Tables["bus"].Columns["sys_num"].ToString();
                dataGridView.Columns["bus_type"].DataPropertyName = myds.Tables["bus"].Columns["bus_type"].ToString();
                dataGridView.Columns["bus_Vm"].DataPropertyName = myds.Tables["bus"].Columns["bus_Vm"].ToString();
                dataGridView.Columns["bus_Va"].DataPropertyName = myds.Tables["bus"].Columns["bus_Va"].ToString();
                dataGridView.Columns["bus_PG"].DataPropertyName = myds.Tables["bus"].Columns["bus_PG"].ToString();
                dataGridView.Columns["bus_QG"].DataPropertyName = myds.Tables["bus"].Columns["bus_QG"].ToString();
                dataGridView.Columns["bus_Vmax"].DataPropertyName = myds.Tables["bus"].Columns["bus_Vmax"].ToString();
                dataGridView.Columns["bus_Vmin"].DataPropertyName = myds.Tables["bus"].Columns["bus_Vmin"].ToString();

                //MessageBox.Show(dataGridView.Rows[dataGridView.CurrentCellAddress.Y].Cells[0].Value.ToString());
                //MessageBox.Show(dataGridView.Rows[dataGridView.CurrentCellAddress.Y].Cells[1].Value.ToString());
            }
            ...
            if (dr == DialogResult.Yes)
            {
                if (start_edit == 0)
                {
                    ...
                    for (int i = del; i >= 1; i--)//从下往上，避免沙漏效应
                    {
                        ...
                        String str = String.Format("delete from bus where bus_index={0}", index0);
        ...
        private void dataGridView_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            if (start_edit == 1)
            {
                ...
                try
                {
                    String str = String.Format("update bus set {0}={1} where bus_index={2}", xl_column, xl_cell, xl_row0);
