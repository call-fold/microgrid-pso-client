    ...
    public partial class Load : Form
    {
        ...
        private void load_Load(object sender, EventArgs e)
        {
            ...
            {//打开菜单配置数据库连接
                ...
                conn.Open();//打开连接
                myadp = new MySqlDataAdapter("select * from loads", conn);
                myds = new DataSet();//填充和绑定数据
                myadp.Fill(myds, "loads");
                dataGridView.AutoGenerateColumns = false;//不生成新的列
                dataGridView.DataSource = myds.Tables["loads"];
                //把数据赋给名称对应的列并显示

                dataGridView.Columns["bus_index"].DataPropertyName = myds.Tables["loads"].Columns["bus_index"].ToString();
                dataGridView.Columns["load_time"].DataPropertyName = myds.Tables["loads"].Columns["load_time"].ToString();
                dataGridView.Columns["load_Pd"].DataPropertyName = myds.Tables["loads"].Columns["load_Pd"].ToString();
                dataGridView.Columns["load_Qd"].DataPropertyName = myds.Tables["loads"].Columns["load_Qd"].ToString();
                dataGridView.Columns["load_num"].DataPropertyName = myds.Tables["loads"].Columns["load_num"].ToString();
            }
        ...
        private void buttonAdd_Click(object sender, EventArgs e)
        {
            if (start_edit == 0)
            {
                try
                {
                    ...
                    myadp.Update(myds.Tables["loads"]);
                }
                catch (MySqlException ex)
                {
                    switch (ex.Number)
            ...
            if (dr == DialogResult.Yes)
            {
                if (start_edit == 0)
                {
                    ...
                    for (int i = del; i >= 1; i--)//从下往上，避免沙漏效应
                    {
                        ...
                        String str = String.Format("delete from loads where load_num={0}", index0);
        ...
        private void dataGridView_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            if (start_edit == 1)
            {
                ...
                try
                {
                    String str = String.Format("update loads set {0}={1} where load_num={2}", xl_column, xl_cell, xl_row0);
