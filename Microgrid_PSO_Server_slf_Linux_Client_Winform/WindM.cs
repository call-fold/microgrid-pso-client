...
{
    public partial class WindM : Form
    {
        ...
        private void windm_Load(object sender, EventArgs e)
        {
            try
            {//打开菜单配置数据库连接
                ...
                conn.Open();//打开连接
                myadp = new MySqlDataAdapter("select * from windm", conn);
                myds = new DataSet();//填充和绑定数据
                myadp.Fill(myds, "windm");
                dataGridView.AutoGenerateColumns = false;//不生成新的列
                dataGridView.DataSource = myds.Tables["windm"];
                //把数据赋给名称对应的列并显示
                dataGridView.Columns["windm_num"].DataPropertyName = myds.Tables["windm"].Columns["windm_num"].ToString();
                dataGridView.Columns["windm_time"].DataPropertyName = myds.Tables["windm"].Columns["windm_time"].ToString();
                dataGridView.Columns["windm_P"].DataPropertyName = myds.Tables["windm"].Columns["windm_P"].ToString();
                dataGridView.Columns["windm_Pmax"].DataPropertyName = myds.Tables["windm"].Columns["windm_Pmax"].ToString();
                dataGridView.Columns["windm_Pmin"].DataPropertyName = myds.Tables["windm"].Columns["windm_Pmin"].ToString();
                dataGridView.Columns["bus_index"].DataPropertyName = myds.Tables["windm"].Columns["bus_index"].ToString();
            }
        ...
        private void buttonAdd_Click(object sender, EventArgs e)
        {
            if (start_edit == 0)
            {
                try
                {
                    ...
                    myadp.Update(myds.Tables["windm"]);
                }
                catch (MySqlException ex)
                {
                    switch (ex.Number)
            ...
            if (dr == DialogResult.Yes)
            {
                if (start_edit == 0)
                {
                    ...
                    for (int i = del; i >= 1; i--)//从下往上，避免沙漏效应
                    {
                        ...
                        String str = String.Format("delete from windm where windm_num={0}", index0);
        ...
        private void dataGridView_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            if (start_edit == 1)
            {
                ...
                try
                {
                    String str = String.Format("update windm set {0}={1} where windm_num={2}", xl_column, xl_cell, xl_row0);
